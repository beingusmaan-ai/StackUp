...
export async function GET(req: NextRequest) {
  ...
  let departmentFilter: { folder: { campaign: { departmentId: string | { in: string[] } } } } | undefined;
  ...
  if (allowedDeptIds !== null) {
    ...
    if (effectiveDeptIds.length > 0) {
      departmentFilter = { folder: { campaign: { departmentId: { in: effectiveDeptIds } } } };
    }
  } else if (rawDeptId) {
    const dept = await db.department.findUnique({ where: { id: rawDeptId }, select: { id: true } });
    if (dept) departmentFilter = { folder: { campaign: { departmentId: dept.id } } };
  }
